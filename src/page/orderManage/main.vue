...
<div class="section">
  <div class="keyword-input">
    <el-row>
      ...
      <el-col :span="10">
          <el-input :span="10" v-model="keyword" size="large" placeholder="请输入支付宝绑定的手机号/订单号/运单号"></el-input>
      </el-col>
  ...
  <el-table :data="tableData" stripe v-loading.body="listLoading" lement-loading-text="拼命加载中" style="width: 100%" max-height="400" :default-sort="{prop: 'date', order: 'descending'}">
    <el-table-column prop="gmtCreate" align="center" label="下单时间" sortable width="180">
    </el-table-column>
    <el-table-column prop="orderNo" align="center" label="相关订单号" sortable width="180">
      <template scope="scope">
          <p>{{cope.row.orderNo}}</p>
          <p>{{cope.row.waybillNo}}</p>          
      </template>
    </el-table-column>
    <el-table-column prop="sendAddress" align="center" label="寄件人信息">
      <template scope="scope">
          <p>{{cope.row.snderName}} {{cope.row.snderPhone}}</p>
          <p>{{cope.row.snderAddress}}</p>          
      </template>
    </el-table-column>
    <el-table-column prop="recipientifo" align="center" label="收件人信息">
      <template scope="scope">
          <p>{{cope.row.rcvrName}} {{cope.row.rcvrPhone}}</p>
          <p>{{cope.row.rcvrAddress}}</p>          
      </template>
    </el-table-column>
...
export default {
  ...
  methods: {
    ...
    loadData: function(){
      var _this =this;
      this.url = "/api/order/getList"; // 默认展开 
